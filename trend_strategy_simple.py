#!/usr/bin/env python3
"""
Trend Strategy Backtest for SPY Historical Data (No Plotting Version)
    ...
    def print_summary(self, results: Dict):
        ...
        if results['total_trades'] > 0:
            ...
            if len(long_trades) > 0:
                print(f"  - Average PnL: ${long_trades['pnl'].mean():,.2f}")
            ...
            if len(short_trades) > 0:
                print(f"  - Average PnL: ${short_trades['pnl'].mean():,.2f}")
            ...
            print(f"  - Actual risk taken: {abs(results['avg_loss'])/self.initial_capital:.2%} per losing trade")
            
            # Performance metrics
            print(f"\nPerformance Metrics:")
            if results['total_return'] > 0:
                print(f"  - Strategy was profitable: YES")
            else:
                print(f"  - Strategy was profitable: NO")
            
            if results['win_rate'] > 0.5:
                print(f"  - Win rate above 50%: YES")
            else:
                print(f"  - Win rate above 50%: NO")
            
            if results['profit_factor'] > 1.0:
                print(f"  - Profit factor above 1.0: YES")
            else:
                print(f"  - Profit factor above 1.0: NO")
